...
스트리밍 토큰에서 `[SPOKEN]...[/SPOKEN]` / `[DISPLAY]...[/DISPLAY]` 분리.

```python
class DualStreamParser:
    # 상태: OUTSIDE | IN_SPOKEN | IN_DISPLAY
    # 태그가 토큰 경계에 걸릴 수 있으므로 내부 버퍼로 누적 후 처리
...
이중 섹션 출력 유도 프롬프트로 재작성.
- `[SPOKEN]`: 구어체 3~5문장, 30자/문장, 마크다운·이모지 금지, 숫자 한글 표기
- `[DISPLAY]`: 길이 제한 없음, 마크다운 자유
- 두 섹션 외 다른 텍스트 금지

### 2-6. `server/cloud/llm_client.py` 수정
- `get_response_stream()` 반환 타입: `AsyncIterator[str]` → `AsyncIterator[tuple[Channel, str]]`
- 내부에서 `DualStreamParser` 사용
- 히스토리 저장: DISPLAY 섹션만 `AIMessage`로 저장 (SPOKEN은 저장 안 함)
